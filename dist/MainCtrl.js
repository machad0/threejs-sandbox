...
function init() {
    ...
    window.util.loadJSON('config.json', function (data) {
        ...
        var elements = data.elements.map(function (element) {
            return new Element(element.url, element.width, element.height, element.x, element.y, element.z, element.ry, element.rx, element.rz);
        });
    ...
    var mouseCtrl = function mouseCtrl(ev) {
    ...
    var gyroSensor = function gyroSensor(ev) {
        ev.preventDefault();
